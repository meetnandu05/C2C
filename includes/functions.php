...
function find_nearby_projects($user_location = null) {
    ...
    $query  = "SELECT * FROM project";
    $query .= " WHERE project_location like '{$user_location}'";
    $query .= " LIMIT 3;";
...
function make_organization_entry($id,$pan) {
    ...
    $pan_number = base64_encode(mysql_prep($pan["pan_number"]));
    $name = base64_encode(mysql_prep($pan["lname"]));
    $dob = base64_encode(mysql_prep($pan["dob"]));
    $address = base64_encode(mysql_prep($pan["flat_building"] . "," . $pan["road_street"] . "," . $pan["area_locality"]));
    $district = base64_encode(mysql_prep($pan["district"]));
    $pin_code = mysql_prep($pan["pincode"]);
    $state = mysql_prep($pan["state"]);
    $image = mysql_prep($pan["image"]);
    $mobile_number=base64_encode($pan["mobile_number"]);
...
function make_individual_entry($id,$aadhaar) {
    global  $conn;
    $aadhaar_number = base64_encode(mysql_prep($aadhaar["aadhaar_number"]));
...
function find_project_progress($project) {
    ...
    $progress = array();
    if (isset($project["project_funds_received"])) {
        $temp = ((int)$project["project_funds_received"] / $project["project_funds"] ) * 100;
        $progress[] = $temp;
    }
    if (isset($project["project_people_hired"])) {
